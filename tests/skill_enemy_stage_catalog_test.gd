...
func run(harness: TestHarness) -> void:
	...
	harness.run_test("free skill catalog matches all 11 Web metadata rows", func() -> void:
...
func _test_free_skills(harness: TestHarness) -> void:
	var expected := [
		...
		["executeStrike", "斩杀", 2, "消耗2技能点。令我方攻击力最高的棋子立即攻击敌方血量最低棋子，造成150%直接伤害；若击杀，回复1点技能点。"],
		["pieceAction", "棋子行动", 1, "消耗1技能点。使指定我方棋子在下次行动时额外行动一次。"],
		["pieceBlock", "棋子格挡", 1, "消耗1技能点。全体我方棋子获得15%临时格挡率，持续1回合。"],
		["pieceDamageUp", "棋子增伤", 1, "消耗1技能点。全体我方棋子直接伤害+30%，持续1回合。"],
	...
	harness.assert_equal(catalog.size(), 11)
...
func _test_hero_abilities(harness: TestHarness) -> void:
	var expected := [
		...
		["ascend", 3, "封命", "消耗2技能点。优先令局外配置的目标位棋子升变为将军（攻击+0、生命上限+80、格挡率+10%、暴击率+5%；若该位置不可用，则回退为存活棋子中序号最小者）。若已存在将军，重复释放使将军攻击+1.5、格挡率+1.5%、暴击率+1.5%，并回复5%已损生命。有我方单位阵亡时，将军获得同层属性成长（不回血）。", "将军出征", 100, "消耗100能量。将军回复已损生命值50%，并获得出征2回合：攻击一列且必定暴击。"],
		...
		["burnEnchant", 5, "炎华", "肉鸽战斗中每场最多投资一次：基础消耗按炎华修习层数为2/3/4技能点，存活非傀儡位置获得引火并立即生效；普通战斗仍消耗2点并施加战斗附魔。", "炎汲仪式", 100, "消耗100能量。全体我方获得炎汲2回合：被带灼烧的敌方攻击时，按目标灼烧层数×0.3%自身已损生命回复。"],
		["fist", 6, "拳劲", "消耗1技能点。造成基于我方棋子平均攻击的直接伤害并叠加拳势（最多5层）。拳势提升直接伤害与暴击率；2/4层时额外攻击1/2个目标。肉鸽中每次成功释放积累1层永久拳意，每层使拳劲与大招伤害+5%。", "拳意·无量", 100, "消耗100能量。随机目标连续打击3段，每层拳势额外+1段，每5层永久拳意使基础段数+1；永久拳意每层使伤害+5%。击杀时追加2段并自动转火，拳势不清空。"],

...
static func _source_definitions() -> Array:
	return [
		...
		Ability.new("ascend", 3, EXCLUSIVE, "封命", "消耗2技能点。首次释放需指定一名存活非傀儡弈子，使其获得将军附魔（生命上限+80、格挡率+10%、暴击率+5%）。已有将军时无需指定，重复释放使其攻击+3、格挡率+3%、暴击率+3%，并回复5%已损生命。", "battle.canCastExclusiveSkill.ascend", "battle.castExclusiveSkill.ascend", false, 0, 2),
		...
		Ability.new("burnEnchant", 5, EXCLUSIVE, "炎华", "本场第1/2/3/4次释放消耗1/2/2/4技能点，之后均消耗4点。使所有可附魔且未满层的存活弈子获得1层炎华附魔。", "battle.canCastExclusiveSkill.burnEnchant", "battle.castExclusiveSkill.burnEnchant", false, 0, 1),
		Ability.new("fist", 6, EXCLUSIVE, "拳劲", "消耗1技能点。造成基于我方弈子平均攻击的直接伤害并叠加拳势。前5层保持原有伤害、暴击率及目标数成长；超过5层后每层额外提供5%拳系增伤。", "battle.canCastExclusiveSkill.fist", "battle.castExclusiveSkill.fist", false, 0, 1),
		...
		Ability.new("fist", 6, ULTIMATE, "拳意·无量", "消耗100能量。随机目标连续打击3段，前5层拳势每层额外+1段并沿用其拳系增伤；5层后的拳势只继续提供每层5%拳系增伤。击杀时追加2段并自动转火，拳势不清空。", "battle.canHeroCastUltimate.fist", "battle.castUltimateByHero.fist", false, 100),

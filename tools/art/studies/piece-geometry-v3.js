export const designNotes = '机弩以两块横向对置棱体围出朝前收窄的负形，在聚拢与错位释放间表达远程攻击；延续青玉与陶土切面，以及刺客的三片轻薄结构。';
...
function crossbow(ctx, p, t) {
  ...
  // Continuous inward gathering, followed by unequal forward displacement.
  // The mouth remains visibly open throughout the motion at small scale.
  const gather = Math.sin(Math.PI * t);
  const release = t * t * (3 - 2 * t);
  ctx.save();
  ctx.translate(-5 * gather + 4 * release, 3 * gather - 2 * release);
  ...
  ctx.save();
  ctx.translate(-4 * gather + 11 * release, -3 * gather + 4 * release);
